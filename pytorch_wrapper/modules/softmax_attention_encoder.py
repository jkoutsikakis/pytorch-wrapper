...
class SoftmaxAttentionEncoder(nn.Module):
    """
    ...
    def __init__(self, attention_mlp, is_end_padded=True):
        ...
        super(SoftmaxAttentionEncoder, self).__init__()
        self._attention_mlp = attention_mlp
        self._is_end_padded = is_end_padded

    def forward(self, batch_sequences, batch_context_vector, batch_sequence_lengths):
        """
        :param batch_sequences: 3D Tensor (batch_size, sequence_length, time_step_size).
        :param batch_context_vector: 2D Tensor (batch_size, context_vector_size).
