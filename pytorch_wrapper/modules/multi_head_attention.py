...
class MultiHeadAttention(nn.Module):
    ...
    def __init__(self, time_step_size, heads, attention_type='dot', dp=0, is_end_padded=True):
        """
    ...
    def forward(self, q, k, v, q_sequence_lengths, k_sequence_lengths):
        ...
        q = q.view(bs, q_len, self._h, self._d_k)
        k = k.view(bs, k_len, self._h, self._d_k)
        v = v.view(bs, v_len, self._h, self._d_k)
        ...
        if self._attention_type == 'multiplicative':
            ...
            scores = torch.matmul(q, scores.transpose(2, 3)) / math.sqrt(self._d_k)
        elif self._attention_type == 'additive':
            q = q.unsqueeze(3).expand(bs, self._h, q_len, k_len, self._d_k)
            k = k.unsqueeze(2).expand(bs, self._h, q_len, k_len, self._d_k)
        ...
        return {'output': attended.view(bs, q_len, self._time_step_size), 'att_scores': scores}
